...
an ideological position, a party endowment (for the ones playing the ``party'' role) and a voter weight (for the ones playing the ``voter'' role). 
 ...
 Each player played the game three times. For every time, a new randomization process took place, i.e., players received a role, an ``ideology,'' a party endowment and voter weight. The total sample size is \Sexpr{as.numeric(nrow(dat.v.b))}. The basic experimental flow is depicted in \autoref{experimental:design:fig}. 
