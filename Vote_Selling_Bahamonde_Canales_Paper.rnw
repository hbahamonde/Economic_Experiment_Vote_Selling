...
%Importantly, both budgets (for party A and B) are the same. However, given that the voter-party distance is assigned at random, both parties have \emph{relative} ``different'' budgets.
...
% parametric model
We proceed by estimating two models. Both consider only the data of participants acting any of the party roles. In the first specification we fit the next OLS regression model,



\begin{equation}\label{eq:1}
\text{Offer} = \beta_{0} + \beta_{1}\text{Vote Share}_{i} + \beta_{2}\delta\text{Points Accumulated}_{i} + \beta_{3}\text{Party Budget}_{i} + \alpha_{i} + \epsilon_{i}
\end{equation}

where \emph{Vote Share} is the certain number of (fictional) voters that were going to cast their votes for the party, $\delta\text{\emph{Points Accumulated}}$ captures changes in the accumulated points respect the experimental round played in $t-1$, \emph{Party Budget} is the party budget. 



In the second specification we fit the next Logit regression model,

\begin{equation}\label{eq:2}

\text{\texttt{logit}}(\pi) = \beta_{0} + \beta_{1}  
