...
\caption{{\bf Distribution of Dependent Variable.\label{dep:var:plot:fig}}}
\caption*{{\bf Note}: Since the focus of this paper is on vote buying, we discarded the voter data (N=\Sexpr{nrow(m1.d)}). Mean = \Sexpr{round(mean(m1.d$offer.made.party),0)}, median = \Sexpr{round(median(m1.d$offer.made.party),0)}, mode = \Sexpr{round(getmode(m1.d$offer.made.party),0)}.}\centering
\end{figure}
...
\caption*{{\bf Note}: Based on the OLS estimates in \autoref{reg:t}, the figure shows the predicted values of the offer made by the party expressed in experimental points. Confidence intervals were constructed robust standard errors (as shown in \autoref{reg:t}). Substantively, the figure shows that experimental subjects avoid losses by over-securing electoral support even in favorable contexts (panel 1) and do consider sunk costs and try to recover losses in the short run by spending more on vote-buying (panel 2). However, parties do not consider ideological/spatial distances with respect to their constituencies when making decisions nor do their budget, i.e., parties with more resources do not necessarily invest more vote buying (panel 3 and 4).}\centering
\end{figure}


Substantive results are shown in \autoref{m1plot} (regression table is shown in \autoref{reg:t}). Overall, results strongly supports prospect theory. Invidials 
