...
% ideology
Voters were assigned an ``ideological'' position at random. That is, voters received at random a certain amount of experimental points depending on whether party A or B won the election. For instance, if party A won the election, a voter would receive in that case 2,400 points, whereas if party B won the election, that voter would receive 200 points. Hence, the voter in this example should feel ``ideologically'' closer to party A. The idea was to model party-voter spatial distances, as considered by traditional voting theories based on the EUT \parencite{Downs:1957vg, Enelow:1990aa}. The basic intuition is that the set of candidates that are ideologically closer to a voter produce more utilities in the form of fiscal policies that are beneficial to that voter, either in terms of redistributive or free-market oriented policies (\cite{Acemoglu:1996rm,Boix:2003db}, but importantly, see \cite{Haggard:2016vr}). Importantly, this piece of information was not presented to subjects as ``ideology.'' Just the points either option would give. In turn, parties also received an ``ideological'' position which made them ``closer'' or ``farther way'' from the voter. The idea was to mimic an electoral market, and observe under which conditions different clientelist dynamics developed. 

% endowments
Both parties received at random different ``endowments'' too. The idea was to reflect the fact that some parties are wealthier than others, a factor that might increase the probability of vote buying \parencite{Juan-Pablo-Luna:2014wp}. Note that both parties receive the same endowment. Since our experimental design is an in between-subjects design, we should be able to observe and exploit the statistical differences (if any) across ``parties'' with different endowments. Their wealth depended on whether they get elected. For every time they buy votes in some amount, that amount is discounted from their wealth. For simplicity, voters received zero endowments.\footnote{This is consistent with the vote-buying literature in that both poor and rich voters are prone to receive clientelist offerings \parencite{Bahamonde2018}.} In turn, their wealth depended on whether their party gets elected and on whether they decided to sell their vote. It is important to note that following our formal model and \textcite[371]{Tyszler2016}, both ideology and party endowments were common knowledge. 

% voter weight
Every voter received a voter weight at random. This ancillary variable was presented to voters as....


an ideological position, a party endowment (for the ones playing the ``party'' role) and a voter weight (for the ones playing the ``voter'' role). 
