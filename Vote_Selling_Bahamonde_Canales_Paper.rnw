...
% questions
In this section we will analyze one main dependent variable, namely, the amount of the vote-buying offer made by parties (if any). The distribution is plotted in \autoref{dep:var:plot:fig}. 
...
In sum, we believe this identification strategy appropriately captures the decision-making process under risk of political parties as theorized by traditional vote-buying scholars based on the EUT. That said, based on prospect theory, we have different expectations, i.e.,  parties should buy votes due to risk-seeking in the domain of losses (i.e., when experienced prior losses) and due to risk-aversion in the domain of gains (i.e., when being a likely winner as the idea of losing electoral support becomes unbearable). 


\begin{figure}[H]
\includegraphics[scale=.6, center]{depvarplot.png}
\caption{{\bf Distribution of Dependent Variable.\label{dep:var:plot:fig}}\\\hspace{\textwidth} {\bf Note}: Note here.}\centering
\end{figure}

% parametric model
Considering only the data of participants $i$ acting any of the party roles, we proceed by fitting the next OLS regression model,
