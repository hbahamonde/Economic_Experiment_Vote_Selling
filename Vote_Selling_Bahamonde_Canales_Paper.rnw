...
While experiments are the gold standard for causal inference \parencite[1349]{Rubin2008c}, laboratory experiments come with an important price, i.e., while some ability to control the environment is gained, some external validity must be sacrificed \parencite{Morton2010}. For instance, our subject pool might not behave in the same way as broker or actual campaigning politicians might do. In addition to that, to gain experimental control, other hard choices in the design had to be made. The most important one is that we did not include in our design the possibility of defecting. As explained above, voters might take the money and then vote for whomever they want. We strongly recomend future research to model different dynamics of clientelist defection. Finally, another limitation is that ``attitudes toward risk are not determined by the S-shaped value function alone'' \parencite[183]{Levy1992a}. Probability weighting is another important feature of prospect theory which unfortunately has been unstudied in political science in general. Future research must consider this important aspect of prospect theory. 
