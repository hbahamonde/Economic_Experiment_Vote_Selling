...
% why an experiment
Building on our formal model of vote buying, a lab economic experiment was designed. Since we intend to contribute to the comparative politics, political development and clientelism literatures, we should note that experiments are the only methodology that can offer strong support for causal inferences \parencite{McDermott2002,DeLaO2011}. 

% administrative
The experiment was conducted in Chile by the \emph{Centre for Experimental Social Sciences (CESS)} administered by the \emph{University of Santiago} and \emph{Oxford University, Nuffield College} between \Sexpr{min(format(as.Date(dat$participant.time_started), format="%B %d %Y"))} and \Sexpr{max(format(as.Date(dat$participant.time_started), format="%B %d %Y"))}. It was programmed in \texttt{O-tree}, the online version of \texttt{Z-tree} \parencite{Fischbacher2007}. Following \textcite{Harrison2006} and others, our experimental design minimizes hypothetical bias and ``cheap talk'' by compensating subjects with real money according to their decisions \parencite{Dickson2011}. Thus, subjects received a show-up fee of CLP \$2,000.\footnote{\textcite[95]{Levy1997} notes that in poorer societies (like the Chilean society) conducting research with relatively smaller monetary incentives is still meaningful. However, see \parencite{Morton:2010ly,Bassi2011}.} The maximum payoff depended on the quality of individual decisions. \autoref{summary:stats:t} shows summary statistics broken down by role and pre-treatment observables (i.e., socio-demographic covariates). A total of \Sexpr{as.numeric(length(unique(v.buying.dat$participant.code)))} subjects were recruited. Formally, we follow an in between-subjects design where different groups of individuals are randomly assigned to various experimental or control conditions \parencite{McDermott2002,Tyszler2016,Hwang2021}. For every game, participants were assigned at random a role, an ideological position, a party endowment (for the ones playing the ``party'' role) and a voter weight (for the ones playing the ``voter'' role). Every game was played among three players (two parties and a voter). Each player played the game three times. For every time, a new randomization process took place, i.e., players received a role, an ``ideology,'' a party endowment and voter weight. The total sample size is \Sexpr{as.numeric(nrow(dat.v.b))}. The basic experimental flow is depicted in \autoref{experimental:design:fig}. 

% role and ideology
Following \textcite{Dickson2011}, roles were presented using a neutral terminology to maximize experimental control. Thus, the design considered the following roles: \emph{party A}, \emph{party B}, or \emph{voter}. Voters were assigned at random an ``ideological'' position. Voters received at random a certain amount of experimental points depending on whether party A or B won the election. For instance, if party A won the election, a voter received 2,400 points, whereas if party B won the election, the voter received 200 points. Hence, the voter in this example should feel ``ideologically'' closer to party A. The idea was to model the spatial distance as considered by traditional voting theories based on the EUT \parencite{Downs:1957vg, Enelow:1990aa}. Importantly, this piece of information was not presented to subjects as ``ideology.'' Just the points either option would give. In turn, parties also received an ``ideological'' position which made them ``close'' or ``farther way'' from the voter. The idea was to mimic an electoral market, and observe under which conditions different clientelist dynamics developed. 


% endowments
Both parties received different endowments too. The idea was to reflect the fact that some parties are wealthier than others, a factor that might increase the probability of vote buying \parencite{Juan-Pablo-Luna:2014wp}. Note that both parties receive the same endowment. Since our experimental design is an in between-subjects design, we should be able to observe and exploit the statistical differences (if any) across ``parties'' with different endowments. For simplicity, voters received zero initial endowments. However, this is consistent with the vote-buying literature in that both poor and rich voters are prone to receive clientelist offerings \parencite{Bahamonde2018}. It is important to note that following our formal model and \textcite[371]{Tyszler2016}, both ideology and party endowments were common knowledge. 
