...
colnames(v.buying.dat.1)[colnames(v.buying.dat.1) == "v.buying.dat.vote_b.1.group.n_votantes"] <- "n.of.voters"
...
colnames(v.buying.dat.2)[colnames(v.buying.dat.2) == "v.buying.dat.vote_b.2.group.n_votantes"] <- "n.of.voters"
...
colnames(v.buying.dat.3)[colnames(v.buying.dat.3) == "v.buying.dat.vote_b.3.group.n_votantes"] <- "n.of.voters"
...
m1 = lm(offer.made.party ~ vote.intention.party.2 + points.cumul.delta + ideo.distance2 + budget + pivotal.voter + participant.code, m1.d)
# options(scipen=9999999) # turn off sci not
# summary(m1)
...
abstract.c = as.character(c("Most traditional theories of clientelism assert that parties in need of securing electoral support invest in vote buying. We consider this framework is limited because it assumes that losses and gains affect a party's decision-making process in a comparable way, and because it assumes that the decision-making process of clientelist political parties focuses only on absolute levels of utilities while overlooking changes in outcomes respect to a reference point. We hypothesize that parties are risk-averse in the domain of gains and risk-seeking in the domain of losses---i.e., losing an election hurts more than winning an election pleases. Unlike traditional theories of clientelism, we argue that vote buying is most likely when parties are probable winners or have experienced important losses in the past. After formalizing a theory of vote buying, we tested it in the lab by designing an economic experiment. Exploiting these novel experimental data, we show that prospect theory bridges important yet unexplained gaps in the literature."))
