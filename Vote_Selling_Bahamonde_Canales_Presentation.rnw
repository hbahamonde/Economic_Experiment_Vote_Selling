...
\begin{frame}[c]%{Treatments}
  \begin{itemize}
    ...
    \item {\bf Parties}: {\color{blue}endowments} (points to buy votes, if any).\\
    {\tiny\color{gray}But both parties receive the same endowment in the same game.}

    \item {\bf Parties}: {\color{blue}vote shares} (number of votes each party \emph{will} receive---excluding the ``voter'' participant.
...
\begin{itemize}
  ...
  \item {\bf Parties}: payoffs depended on whether they get elected. If they buy votes, that amount is discounted. % For simplicity, voters received zero endowments

  \item {\bf Voters}: payoffs depended on whether their party get elected, and on whether they sell their vote. {\bf Can't defect: if they accept and offer, they vote for that party}.
\end{itemize}
\end{column}
...
\begin{itemize}
  \item Our focus is vote buying. \\
  Voter data discarded. 

  \item {\bf Dependent variable}: the amount of the vote-buying offer made by parties (if any).

  \item If predictions of formal model and traditional vote-buying th. is accurate, we should see that offers go up in risky scenarios (facing probable electoral losses).
\end{itemize}
\end{column}
...
\includegraphics[scale=0.4, center]{depvarplot.png}
