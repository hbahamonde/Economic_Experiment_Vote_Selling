...
\begin{itemize}
    \item {\bf Say you're a \underline{clientelist} political party}\pause:
        \begin{enumerate}
    ...
    \item {\bf If we follow what the literature says...the answers are mixed}:\pause
        \begin{enumerate}
            ...
            \item[\checkmark] It shouldn't: {\bf ``sunk costs''} should \emph{not} affect current decisions.\pause
        \end{enumerate}
     \item {\bf And yet...}:
        \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item Risk-aversion in the domain of {\color{dkgreen}{\bf gains}}:
        \begin{itemize}
          ...
          \item[\checkmark] Deal with core voters (parties would hate to lose already {\color{dkgreen}acquired assets})\pause
        \end{itemize}
...
\begin{frame}[c]{Clientelism and the Expected Utility Theory}
  \begin{itemize}
    ...
    \item {\bf The problem}. The literature assumes:\pause
              \begin{enumerate}
...
\begin{frame}{Not Clear Who Clientelist Parties Target}
  \begin{itemize}
    ...
    \item Since allocating resources to individuals who ex-ante vote for the party is a waste, parties should target {\bf swing voters}.\\
...
\begin{itemize}[<+->]
      \item[2.] {\bf Value function}. The asymmetrical curvature of the value function influences decisions:
          \begin{itemize}
...
\begin{enumerate}%[<+->]
      ...
      \item Since past loses alter the reference point, incumbents will buy more votes when they've spent/lost a lot in the past.
        \begin{itemize}
          \item[\checkmark] {\bf Sunk costs are high}.
        \end{itemize}
