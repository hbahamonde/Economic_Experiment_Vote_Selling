...
\vspace{1cm}
  ...
  \begin{itemize}
    ...
    \item[$\star$] {\bf Supply and demand story}: Do parties target likely voters? Why? At what price? Under what conditions sellers their votes?
  \end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item Two candidates, one ``left-wing'' party and one ``right-wing'' party, which represents a policy which is an \emph{iid} draw from an uniform distribution over $\gamma_L \in \{1,...,50\}$ ($\gamma_R \in \{51,...,100\}$) .
	\item There are $n_L$ supporters of ``left-wing'' party and $n_R$ of ``right-wing'' party.
	\item Both parties negotiate with only one of these $n$ voters, which is randomly selected from the total population.
\end{itemize}
\end{frame}
...
\item The profits of party $i$ is given by,
...
\begin{itemize}
	...
	\item The voter he voter decides if to take the offer, or which one accept if he receives two offers.
	\item Voting is realized, if the voter accept an offer he should vote for this party.
	\end{itemize}
		\item \textbf{Vote-selling Case}
		\begin{itemize}
	\item The voter may privately propose a certain amount to each party in exchange for his vote.
	\item The parties decide if to pay or not the offer.
	\item The voter then decides which one to accept, if any.
	\item  Voting is realized, if the voter accept an offer he should vote for this party.
	\end{itemize}
...
\begin{frame}{Equilibrium in Vote-Buying Case}
\begin{itemize}
	\item Parties want to win the election at a minimum cost then, in equilibrium $m^*_{i^*}=0$ and $m^*_{-i^*}=\vert x_{i^*}-\gamma_{-i^*} \vert- \vert x_{i^*}-\gamma_{i^*} \vert$.
	\item The pivotal voter indifferent between both political parties.
	\item Two Nash Equilibrium,
	\begin{itemize}
...
\begin{frame}{Equilibrium in Vote-Selling Case}
	\begin{itemize}
		...
		\item The voter may swing towards party $-i^*$ only if the budget is big enough to compensate what he looses when voting for his less prefer policy ($B> \vert x_{i^*}-\gamma_{-i^*} \vert- \vert x_{i^*}-\gamma_{i^*} \vert$).
...
\end{center}
\vspace{0.8cm}
\item Nash Equilibrium: $\{(B,B), (\text{Accept}, \text{Accept}), \text{Accept offer } i^* \}$
	\end{itemize}
\end{frame}
