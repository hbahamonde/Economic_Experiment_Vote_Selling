...
\begin{itemize}
    \item {\bf Say you're a \underline{clientelist} political party}\pause:
        \begin{enumerate}
            ...
            \item Who do you target? {\scriptsize\color{gray} Your own supporters or the ones who are more likely to flip?}\pause
            \item Do prior electoral losses matter?\pause
        \end{enumerate}
...
\centering {\bf Vote-buying will be higher when parties}...\vspace{1cm}
    ...
    \begin{itemize}
      ...
      \item Risk-aversion in the domain of {\color{dkgreen}{\bf gains}}:
        \begin{itemize}
          \item[\checkmark] \alert<+(1)>{Are {\color{dkgreen}wining} the election}.
          \item[\checkmark] \alert<+(2)>{Deal with core voters (parties would hate to lose already {\color{dkgreen}acquired assets})}
        \end{itemize}

          \item Risk-seeking in the domain of {\color{red}{\bf losses}}.
            \begin{itemize}
              \item[\checkmark] \alert<+(3)>{Have experienced {\color{red}losses} in the past (sunk costs)}.
            \end{itemize}
