...
\begin{enumerate}%[<+->]
      \item Due to {\bf loss aversion} parties will find {\bf intolerable} the idea of {\bf losing the supporter base they already have}.
        \begin{itemize}
          ...
          \item[\checkmark] {\bf When dealing with own supporters}.\pause
        \end{itemize}
...
\begin{frame}[c]%{Experimental Design: Setup}
  \begin{itemize}
    ...
    \item All transactions were performed exchanging experimental ``points.'' {\tiny\color{gray}1 point = \$\Sexpr{as.numeric(unique(dat$session.config.real_world_currency_per_point))}.}
...
\begin{frame}%{Payoffs}
  ...
  \begin{column}{.48\textwidth}
      \begin{itemize}
        ...
        \item[] {\color{blue}Risk aversion: It hurts more to lose {\bf closest} supporters}.
      \end{itemize}
\end{column}
\hfill%
  \begin{column}{.48\textwidth}
...
\begin{frame}{Main Takeaway}
  ...
  \begin{itemize}
    ...
    \item Parties don't see vote buying in the typical ``{\bf insurance}'' sense: {\color{blue}parties don't buy more votes in {\bf tighter electoral races}}.

    \item {\color{blue}Decision-makers are more concerned with {\bf preventing} a decline than {\bf increasing} gains}.
  \end{itemize}
