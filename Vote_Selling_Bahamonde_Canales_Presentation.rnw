...
\begin{itemize}
    \item {\bf Say you're a \underline{clientelist} political party campaigning}\pause:
        \begin{enumerate}
            \item When do you buy votes? {\scriptsize\color{gray} Winning/losing the election?}\pause
    ...
    \item {\bf If we follow what the literature says...the answers are mixed}:
        ...
        \end{enumerate}
   \item {\bf We contest these answers}. \pause Explain: {\color{blue}traditional clientelism research has failed to answer these questions because it has a wrong understanding about the party's decision-making process under risk}.
   \end{itemize}
\end{frame}
...
\begin{frame}[c]{Milestones}
    \begin{itemize}
      ...
      \item {\bf Empirics}: we formalized a vote buying game, and then test it in an economic lab experiment.\pause
...
\begin{frame}{Not Clear Who Clientelist Parties Target}
  \begin{itemize}
    ...
    \item {\color{blue}We contend that this is a \emph{very} important question, yet one that the literature has \emph{failed} to answer}.
  \end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item If predictions of formal model are accurate, we should see that offers go up when the risk of losing the election is higher.
\end{itemize}
\end{column}
