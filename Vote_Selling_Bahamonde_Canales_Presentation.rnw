...
\begin{enumerate}[<+->]
      \item {\bf Are probable winners}: due to loss aversion, parties will find intolerable the idea of losing the supporter base they already have ({\color{blue} risk-aversion; decision-makers are more concerned with preventing a decline than increasing gains}).
...
\includegraphics[scale=0.028, center]{payoffplot.png}
...
\subsection{Dependent Variable and Predictions}
...
%\item {\bf Predictions}:
  %\begin{%}
    ...
    \item EUT: parties focus on {\bf wins}, i.e., offers go up when parties are {\bf losing the election}.

    \item PT: parties focus on {\bf loses} {\color{blue}(don't want to lose what they ``own'')}.
      \begin{enumerate}
        \item Buy more votes when the parties are {\bf wining} the election.
        \item Buy more votes from {\bf core} supporters (hurts more to lose \emph{\bf closest} voters).
        \item Buy more votes when yesterday's costs are high (need to spend  more to ``\emph{break even}''). 
      \end{enumerate}
  \end{itemize}
%\end{itemize}
\end{column}
...
\begin{frame}[c]
        ...
        \begin{itemize}
            \vspace{-1.5cm}\item[3.] Parties buy more votes at higher (not lower) prices from closest supporters.
        \end{itemize}
\end{frame}
...
\begin{frame}[c]
        ...
        \begin{itemize}
            \vspace{-1.5cm}\item[4.] Parties with larger budgets spend more on vote buying.
        \end{itemize}
\end{frame}
...
\begin{frame}[c]
        ...
        \begin{itemize}
            \vspace{-1.5cm}\item[5.] Pivotal votes don't cost more (against most of spatial theories of voting).
        \end{itemize}
\end{frame}
...
\begin{frame}%{Payoffs}
    \begin{itemize}
    \item This paper identified {\bf three main gaps} in the literature. We don't know,
      \begin{enumerate}
        \item {\color{blue}the rationale of clientelist targeting}.
        \item {\color{blue}the role of electoral contestation}.
        \item {\color{blue}the \underline{unstudied} role of sunk costs}.\pause
      \end{enumerate}
    ...
    \item PT explains better the gaps in the literature.
  \end{itemize}
\end{frame}%{Payoffs}
