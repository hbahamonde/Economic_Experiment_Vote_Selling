...
\begin{itemize}
    ...
    \item {\bf If we follow what the literature says...the answers are mixed}:\pause
     ...
     \item {\bf And yet...}:
        \begin{enumerate}
            \item[?] Incumbents buy votes when they're {\bf winning} {\scriptsize{\color{gray}(Gonzalez-Ocantos, 2012)}}.\pause
            \item[?] Why targeting core voters isn't a {\bf waste}?\pause
...
\centering {\bf Vote-buying will be higher when parties}...\vspace{1cm}
    ...
    \begin{itemize}
      ...
      \item Risk-aversion in the domain of {\color{dkgreen}{\bf gains}}:
        \begin{itemize}
          \item[\checkmark] \alert<+(1)>{Are {\bf wining} the election}.
          \item[\checkmark] \alert<+(2)>{Deal with {\bf core voters}}. % (parties would hate to lose already {\color{dkgreen}acquired assets})
        \end{itemize}

          \item Risk-seeking in the domain of {\color{red}{\bf losses}}.
            \begin{itemize}
              \item[\checkmark] \alert<+(3)>{Have experienced {\bf losses} in the past (sunk costs)}.
            \end{itemize}
  ...
  \begin{itemize}
    ...
    \item {\bf The problem}. This approach assumes:\pause
        ...
        \item These assumptions lead to several inconsistencies.\pause
          \begin{enumerate}
            \item The mechanics of {\bf clientelist targeting}.
            \item The role of {\bf political contestation} on vote buying.
          \end{enumerate}
  \end{itemize}
\end{frame}
