...
\begin{enumerate}[<+->]
      \item {\bf Are probable winners}: due to {\bf loss aversion}, parties will find {\bf intolerable} the idea of {\bf losing the supporter base they already have} ({\color{blue} decision-makers are more concerned with preventing a decline than increasing gains}; risk-aversion).
