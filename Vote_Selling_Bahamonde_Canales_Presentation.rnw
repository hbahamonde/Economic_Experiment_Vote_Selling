...
\begin{frame}{Plan for Today}
    \begin{itemize}
      \item {\bf Motivate the problem}: vote buying literature is based solely on the Expected Utility Theory {\tiny (von Neumann and Morgenstern)}.
      ...
      \item {\bf Empirics}: following the precepts of EUT, we formalized a vote buying game, and then test it in an economic experiment.
    ...
    \begin{itemize}
      ...
      \item Safe to say that 99\% of the vote-buying literature takes the Expected Utility Theory as a starting point:
          \begin{itemize}
            ...
            \item Consequences, the literature assumes that in a party's decision-making process,
              \begin{itemize}
                  \item losses and gains affect in a comparable way (winning elections feels good as losing one hurts).
                  \item parties focus only on absolute levels of utilities (while overlooking changes in outcomes respect to a reference point).
              \end{itemize}
          \end{itemize}
    \end{itemize}
\end{frame}
