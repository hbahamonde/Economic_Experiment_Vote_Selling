...
\begin{itemize}
    \item {\bf Say you're a \underline{clientelist} political party campaigning}\pause:
        \begin{enumerate}
            \item When do you buy votes? {\scriptsize\color{gray} Winning/losing the elections?}\pause
            \item Who do you target? {\scriptsize\color{gray} Your own supporters (``core'') or the ones who are more likely to flip (``swing'')?}\pause
            \item Should your past ``haunt'' you? {\scriptsize\color{gray} Do prior losses matter?}\pause
        \end{enumerate}
    \item {\bf Intuitively, these questions seem easy to answer}:
        \begin{enumerate}
            \item[\checkmark] When losing the elections: {\bf risk}.\pause
            \item[\checkmark] To the ones who are more likely to flip (``swing''): {\bf waste}.\pause
            \item[\checkmark] It shouldn't: {\bf ``sunk costs.''} should not affect current decisions.\pause
        \end{enumerate}
   \item {\bf They are not}. \pause Starting point: {\color{blue}traditional clientelism research has failed to answer these questions because it has a wrong understanding about the decision-making process of clientelist parties}.
   \end{itemize}
\end{frame}
...
\begin{frame}[c]{Plan for Today}
    \begin{itemize}
      \item {\bf Motivate the problem}: vote buying literature is purely based on the Expected Utility Theory (EUT) {\tiny (von Neumann and Morgenstern)}.\pause
      \item {\bf Explain why we should care}: there are too many important loose ends.\pause
      \item {\bf Propose a possible solution}: re-think about how parties make decisions under risk (Prospect Theory).\pause
      \item {\bf Empirics}: follow the concepts of EUT, we formalized a vote buying game, and then test it in an economic lab experiment.\pause
      \item {\bf Results}: we find strong support in favor of prospect theory.\pause
...
\begin{frame}[c]{Argument}
   ...
   \centering
    \begin{itemize}
      \item[\checkmark] {\color{blue} Are probable winners}---{\color{gray}risk averse in the domain of gains}.\pause
...
\begin{frame}[c]{Clientelism and the Expected Utility Theory}
  \begin{itemize}
    \item The EUT was one of the first theories of decision making under risk.\pause
    \item Since its introduction, it has \emph{dominated} political science as a field (including the vote-buying lit.).\pause
    \item {\bf The problem}: the (whole!) literature assumes that in the party's decision-making process:\pause
              \begin{enumerate}
                  \item {\color{blue}Losses and gains affect in a {\bf comparable} way}.\\
                  {\scriptsize\color{gray}Winning elections {\bf feels good as} losing one hurts.}\pause
              ...
              \end{enumerate}
        \item These assumptions have led to several empirical inconsistencies.\pause
          \begin{enumerate}
...
\begin{itemize}[<+->]
      \item[1.] {\bf Reference dependence}. Elements that influence decisions,
          \begin{itemize}
            \item[\checkmark] {\color{blue}context} in which the decision-making processes take place.
            \item[\checkmark] {\color{blue}changes of wealth}, rather than final asset positions.
...
\begin{itemize}[<+->]
      \item[2.] {\bf Likelihood dependence}. Asymmetrical curvature of the likelihood influences decisions,
          \begin{itemize}
            \item[\checkmark] Individuals are risk-averse in the domain of gains.
            \item[\checkmark] Individuals are risk-acceptant in the domain of losses. 
          \end{itemize}
\end{itemize}
...
\begin{itemize}[<+->]
      \item {\bf Overweighting}: in the domain of {\color{blue}losses}, the convexity of the  function encourages {\color{blue}risk-seeking} behaviors by {\bf exaggerating the probabilities of unlikely losses}.
      \item {\bf Underweighting}: in the domain of {\color{blue}gains}, the concavity of the function encourages {\color{blue}risk aversion} by {\bf undervaluing the probability of success}.
\end{itemize}
...
\begin{enumerate}[<+->]
      \item {\bf Are probable winners}: parties will exaggerate the small probability of losing the election ({\color{blue}risk aversion}).
      \item {\bf Have experienced losses in the past} (sunk costs): alter the decision-makers' reference point, making vote-buying an attractive strategy ({\color{blue}risk-seeking}).
...
\begin{frame}{A Formal Model of Vote-Buying}
  \begin{itemize} 
        ...
        {\tiny\color{gray}McDermott (2002), Aldrich and Lupia (2011), Barberis (2013).} \pause
        \item We developed a vote-buying game within the Downsian-``spatial'' paradigms (EUT).\\\pause
         {\tiny\color{gray}Downs (1957), Enelow and Hinich (1990), Plott (1991).} 
        \item The idea is to test the descriptive accuracy of the game-theory model in an experimental setting.\\
...
\begin{frame}%{Payoffs}
    \begin{itemize}
    \item This is a work in progress.\pause  
    \item In this paper identified {\bf two main gaps} in the literature. We don't know,
      \begin{enumerate}
        ...
        \item The role of electoral contestation.\pause
      \end{enumerate}
    \item We {\bf attribute this confusion} to the the fact that the literature takes the {\bf EUT as a starting point}.\pause
    \item We introduced {\bf Prospect Theory} in the vote-buying literature as an {\bf alternative way to understand decision-making under risk}.\pause
    \item To test this theory, we {\bf formalized a theory of vote-buying} and test it in the {\bf lab}.\pause
