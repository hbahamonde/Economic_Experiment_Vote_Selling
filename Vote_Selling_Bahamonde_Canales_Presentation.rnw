...
\subsection{Motivating the problem}
...
\begin{itemize}
    ...
    \item {\bf Intuitively, these questions seem easy to answer}:
        \begin{enumerate}
            ...
            \item[\checkmark] It shouldn't: prior losses should not matter: {\bf ``sunk costs.''}
        \end{enumerate}
...
\begin{frame}[c]%{Experimental Design: Setup}
  \begin{itemize}
    ...
    \item  A total of \Sexpr{as.numeric(length(unique(v.buying.dat$participant.code)))} subjects were recruited. 
...
\begin{frame}[c]%{Treatments}
  \begin{itemize}
    ...
    \item {\color{blue}Role}: \emph{party A}, \emph{party B} or \emph{voter}.

    \item {\bf Voters}: {\color{blue}``ideological position''} (points depending on whether party A or B won election). 
    ...
    \item {\bf Parties}: {\color{blue}endowments}.\\
    {\tiny\color{gray}But both parties receive the same endowment in the same game.}

    \item {\bf Parties}: {\color{blue}vote shares} (certain number of votes each party will receive---excluding the ``voter'' participant.
...
\begin{enumerate}
    ...
    \item Participants acting the “party” role accumulated/lost wealth depending on whether they got elected/not-elected. For every time they bought votes at some amount, that amount was discounted from their wealth. For simplicity, voters received zero endowments

    \item participants acting the “voter” role accumulated/lost wealth depending on whether their party got elected/not-elected (as per traditional EUT voting theories) and on whether they decided to sell/not-sell their vote.


    \item Payoffs depended on...
