...
{\bf Vote buying}: distribution of private rewards to individuals during elections in exchange for electoral support {\scriptsize(Nichter, 2014)}.
\vspace{1cm}
...
\begin{itemize}
    \item {\bf Say you're a \underline{clientelist} political party campaigning}\pause:
        \begin{enumerate}
            ...
            \item Should your past ``haunt'' you? {\scriptsize\color{gray} Do prior electoral losses matter?}\pause
        \end{enumerate}
    \item {\bf If we follow what the literature says...the answers are mixed}:
        \begin{enumerate}
            ...
            \item[\checkmark] To both core and swing voters  (targeting core supporters is not a {\bf waste}).\pause
            \item[\checkmark] It shouldn't: {\bf ``sunk costs''} should not affect current decisions.\pause
        \end{enumerate}
   \item {\bf We contest these answers}. \pause Explain: {\color{blue}traditional clientelism research has failed to answer these questions because it has a wrong understanding about the decision-making process of clientelist parties}.
   \end{itemize}
\end{frame}
...
\begin{frame}[c]{Milestones}
    \begin{itemize}
      ...
      \item {\bf Problem}: as a consequence, there are too many important loose ends.\pause
...
\begin{frame}{Not Clear The Role of Political Contestation on Vote Buying}
  \begin{itemize}
    \item The more contested an election, the more risks of losing the election, the more vote buying.\\
...
\begin{itemize}[<+->]
      \item[2.] {\bf Likelihood dependence}. The asymmetrical curvature of the likelihood does influence decisions,
          \begin{itemize}
...
\miniframeson
\begin{frame}{Prospect Theory: Implications for Decision-Making}
  \begin{columns}[c]
  \begin{column}{.48\textwidth}
...
\begin{enumerate}[<+->]
      ...
      \item {\bf Have experienced losses in the past} (sunk costs): alter the  reference point, making vote-buying an attractive strategy ({\color{blue}risk-seeking}).
...
\begin{frame}[c]%{Experimental Design: Setup}
  \begin{itemize}
    ...
    \item In-between subjects experimental design.
...
\begin{itemize}
  ...
  \item If predictions of formal model and traditional vote-buying theories are accurate, we should see that offers go up when the risk of losing the election is higher.
\end{itemize}
\end{column}
...
\begin{frame}%{Payoffs}
    ...
    \item In this paper identified {\bf two main gaps} in the literature. We don't know,
      \begin{enumerate}
        \item {\color{blue}The rationale of clientelist targeting}.
        \item {\color{blue}The role of electoral contestation}.\pause
      \end{enumerate}
