...
\begin{itemize}
    ...
    \item {\bf If we follow what the literature says...the answers are mixed}:\pause
     ...
     \item {\bf And yet...}:
        \begin{enumerate}
            \item[?] Incumbents also buy votes when they're {\bf winning} the election {\scriptsize{\color{gray}(Gonzalez-Ocantos, 2012)}}.\pause
...
%\item {\bf Predictions}:
  %\begin{%}
    ...
    \item PT: parties focus on {\bf loses} {\color{blue}(hold on to what they ``own'')}.
      \begin{enumerate}
        \item Buy more votes when parties are {\bf wining} the election.
        \item Buy more votes from {\bf core} supporters (\emph{hurts more to lose} \emph{\bf closest} \emph{voters}).
